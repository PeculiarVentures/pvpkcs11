...
AttributeInfo pAttrInfo[] = {
    ...
    {CKA_X509_CHAIN, "CKA_X509_CHAIN", PVT_ATTRIBUTE_BYTES},
};

std::string GetAttributeName(CK_ATTRIBUTE_TYPE type)
{
    LOGGER_FUNCTION_BEGIN;

    for (CK_ULONG i = 0; i < sizeof(pAttrInfo) / sizeof(AttributeInfo); i++)
    {
...
AttributeBytes::AttributeBytes(
    ...
    CK_BYTE_PTR pData,
    CK_ULONG ulDataLen,
    CK_ULONG flags) : AttributeTemplate(type, pData, ulDataLen, flags)
{
}
...
AttributeBool::AttributeBool(
    CK_ATTRIBUTE_TYPE type,
    CK_BBOOL bData,
    CK_ULONG flags) : AttributeBytes(type, &bData, sizeof(CK_BBOOL), flags)
{
}
...
AttributeNumber::AttributeNumber(
    CK_ATTRIBUTE_TYPE type,
    CK_ULONG ulData,
    CK_ULONG flags) : AttributeBytes(type, (CK_BYTE_PTR)&ulData, sizeof(CK_ULONG), flags)
{
}
...
{
    LOGGER_FUNCTION_BEGIN;
    auto attrName = GetAttributeName(type);
    LOGGER_DEBUG("%s type:%s", __FUNCTION__, attrName.c_str());
    ...
    THROW_PKCS11_EXCEPTION(CKR_GENERAL_ERROR, "Cannot get attribute %s", attrName.c_str());
}
...
bool Attributes::HasAttribute(CK_ATTRIBUTE_TYPE type)
{
    LOGGER_FUNCTION_BEGIN;
...
void Attributes::Add(Scoped<Attribute> item)
{
    LOGGER_FUNCTION_BEGIN;
