...
osx::RsaPKCS1Sign::RsaPKCS1Sign(CK_BBOOL type) : core::CryptoSign(type)
{
    ...
    CK_ULONG_PTR pulSignatureLen)
{
  ...
  try
  {
    ...
    else
    {
      ...
      Scoped<SecKey> secKey = key->Get();
      Scoped<CFData> signature = secKey->CreateSignature(
          keyAlgorithm,
          cfHash->Get());
    ...
    CK_ULONG ulSignatureLen)
{
  ...
  try
  {
    ...
    Scoped<CFData> cfHash = CFData::Create(kCFAllocatorDefault, hash, hashLen);
    Scoped<CFData> cfSignature = CFData::Create(kCFAllocatorDefault, pSignature, ulSignatureLen);

    Scoped<SecKey> secKey = key->Get();
    Boolean ok = secKey->VerifySignature(
        keyAlgorithm,
        cfHash->Get(),
        cfSignature->Get());
