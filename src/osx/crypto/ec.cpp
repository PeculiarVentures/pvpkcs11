...
osx::EcDsaSign::EcDsaSign(CK_BBOOL type) : core::CryptoSign(type)
{
}
    ...
    CK_ULONG_PTR pulSignatureLen)
{
  ...
  try
  {
    ...
    CFRef<CFDictionaryRef> attrs = SecKeyCopyAttributesEx(key->Get()->Get());
    ...
    else
    {
      ...
      Scoped<CFData> cfHash = CFData::Create(kCFAllocatorDefault, hash, hashLen);

      Scoped<CFData> signature = key->Get()->CreateSignature(
          keyAlgorithm,
          cfHash->Get());

      active = false;

      Scoped<Buffer> wcSignature = ConvertSignatureToWebcrypto(signature->Get(), keySizeInBits);
    ...
    CK_ULONG ulSignatureLen)
{
  ...
  try
  {
    ...
    Scoped<CFData> cfHash = CFData::Create(kCFAllocatorDefault, hash, hashLen);
    Scoped<CFData> cfWebcryptoSignature = CFData::Create(kCFAllocatorDefault, pSignature, ulSignatureLen);
    Scoped<Buffer> derSignature = ConvertSignatureFromWebcrypto(cfWebcryptoSignature->Get());
    Scoped<CFData> cfSignature = CFData::Create(kCFAllocatorDefault, derSignature->data(), derSignature->size());

    Boolean ok = key->Get()->VerifySignature(
        keyAlgorithm,
        cfHash->Get(),
        cfSignature->Get());
